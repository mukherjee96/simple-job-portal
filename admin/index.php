<?php 
   ...
   if(!isset($_SESSION['admin']))
      ...
      if(isset($_REQUEST['mode'])) {
         if($_REQUEST['mode'] == 'jobseeker')
            $mode = 'jobseeker';
         else if($_REQUEST['mode'] == 'highlighted-job')
            $mode = 'highlighted-job';
      }
      else
         $mode = 'jobseeker';
   }
   ...
   if(isset($_REQUEST['delete'])) {
      ...
      $sql = "DELETE FROM jobseeker WHERE email='".$email."'";
      $response = $con->exec($sql);
      echo'<script>window.location.href="index.php?mode=jobseeker"</script>';
   }

   //Grant
   if(isset($_REQUEST['grant'])) {
      $email = $_REQUEST['email'];
      $sql = "UPDATE jobseeker SET verified='true' WHERE email='".$email."'";
      $response = $con->exec($sql);
      echo'<script>window.location.href="index.php?mode=jobseeker"</script>';
   }
   ...
   if(isset($_REQUEST['revoke'])) {
      $email = $_REQUEST['email'];
      $sql = "UPDATE jobseeker SET verified='false' WHERE email='".$email."'";
      $response = $con->exec($sql);
      echo'<script>window.location.href="index.php?mode=jobseeker"</script>';
   }
   ...
   if(isset($_REQUEST['highlight'])) {
      ...
      if($_REQUEST['highlight'] == 'true')
         $sql = "UPDATE jobs SET highlighted='true' WHERE id='".$id."'";
      else
         $sql = "UPDATE jobs SET highlighted='false' WHERE id='".$id."'";
      $response = $con->exec($sql);
      echo'<script>window.location.href="index.php?mode=highlighted-job"</script>';
   }

   //Search job Seekers
   if(isset($_POST['jobseeker-search'])) {
      echo'<script>window.location.href="index.php?mode=jobseeker&search='.$_POST['jobseeker-search'].'"</script>';
   }

   //Search Jobs
   if(isset($_POST['job-search'])) {
      echo'<script>window.location.href="index.php?mode=highlighted-job&search='.$_POST['job-search'].'"</script>';
   }
?>
  ...
  <body>
      ...
      <div class="container-fluid mt-4">
         <div class="d-flex mb-3">
            <div class="mr-auto p-2"><h4>Job Seeker Management</h4></div>
      ...
      <div class="container-fluid mt-4">
         <div class="d-flex mb-3">
            <div class="mr-auto p-2"><h4>Highlight Jobs</h4></div>
            <div class="p-2 w-50"><input type="text" class="form-control" name="job-search" id="job-search" placeholder="Search by Designation or Technology"></div>
         </div>
